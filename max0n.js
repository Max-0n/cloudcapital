...
max0n.arrowMakeUp = function(arr){ //функция запускающая прорисовку каждой стрелки
	var canvDraw = document.getElementById(arr[0]).getContext('2d'); //получаю 2D холст нашего канваса по имени свойства
	pic = new Image();	//создаю изображение для нарисовки
	pic.src = 'img/arrow.png'; //по данной ссылке
	var maximal = 0; //максимальное значение массива, обнулено
	for(var j=1;j<arr.length;j++){ //присвоение максимального значения массива
		if(maximal<arr[j]) maximal=arr[j];
	}
	max0n.animateArrow(canvDraw,pic,maximal,arr);
	/*for(var j=1;j<arr.length;j++){ //запустить прорисовку каждой стрелки
		max0n.animateArrow(canvDraw,pic,((800/arr.length)*j-12.5),200-arr[j]/maximal*200);
	}*/
}
max0n.animateArrow = function(elem,image,maximal, y){ //функция прорисовки каждой стрелки
	animate({
		duration: 1500,
		delta: makeEaseOut(quint),
		from: 200,
		to: y,
		step: function(delta,from,to) {
			elem.clearRect(0, 0, 800, 200); //очисть блок прорисовки
			(function(to){//замыкание отрисовки стрелок
				for(var j=1;j<to.length;j++){ //нарисовать каждую стрелку
					elem.drawImage(image, (800/to.length)*j-12.5, ((delta*((200-to[j]/maximal*200)-from)) + from))
				}
			})(to); //конец замыкания
		}
	});
